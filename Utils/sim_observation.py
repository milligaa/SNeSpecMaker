...
def Comb_Maker(
    ...
    :param str name: ID of the spectrum.
    ...
    :param dict SALT2_params: dictionary containing SN phase, x0, x1 and c.
    :return: the SNR of host and SN combined, the new magnitude of the SN, 
             the observed wavelength array, the observed flux array.
    :rtype: list
    """
    ...
    if SNe_data == 'SALT2':
        print(f'Performing SALT2 Spec Creation on SN: {name}')
        salt2 = sncosmo.get_source(name='salt2', version='2.0')
        params = {'c': SALT2_params['c'], 'x1': SALT2_params['x1'], 'x0': SALT2_params['x0']}
        salt2.update(params)

        lam = np.linspace(2000, 9200, 7201)
        intens = salt2.flux(phase=SALT2_params['phase'], wave=lam)
    else:
        #importing the data to be used
        print(f'Performing Non-Ia Spec Creation on SN: {name}')
    ...
    gal_spec = fits.open(Gal_data)
    spec_data2 = gal_spec[1].data

    lam2 = []
    intens2 = []
    for i in range(len(spec_data2)):
        lam2.append(spec_data2[i][0])
        intens2.append(spec_data2[i][1]) 
    ...
    name_of_file = spec_save_path+gt+st+'Smag'+sm+'Gmag'+gm+'name'+str(name)+'z'+re+'.txt'

    print(name_of_file)

    ascii.write(comb_table, name_of_file, format = 'no_header', overwrite = True)
